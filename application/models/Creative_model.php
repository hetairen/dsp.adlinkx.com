...
class Creative_model extends ADLINKX_Model {
	...
	public function lists($where = array(), $offset = 0, $num = 20, $key = 'id', $stor = 'desc', $fields = '*', &$count) {
		...
		$sql = "SELECT `dat`.`id`,`dat`.`borad_id`, `dat`.`borad_name`, `dat`.`unit_id`, `dat`.`unit_name`, `dat`.`plan_id`, `dat`.`plan_name`, `dat`.`shop_id`, `dat`.`plat_id`, `dat`.`plat_name`, `dat`.`uid`, `dat`.`status`, `dat`.`pic_path`,`dat`.`pic_size`,`dat`.`borad_url`, `s`.`shop_title`, IFNULL(SUM(`ds`.`pv`), 0) AS 'pv', IFNULL(SUM(`ds`.`click`), 0) AS 'click', IFNULL(SUM(`ds`.`charge`), 0) AS 'charge', IFNULL(SUM(`ds`.`click`) / SUM(`ds`.`pv`) * 100, 0) AS `ctr`, IFNULL(SUM(`ds`.`charge`) / SUM(`ds`.`click`), 0) AS `click_cost` FROM (`diy_ad_task` AS `dat`) LEFT JOIN `store` AS `s` ON s.shop_id = dat.shop_id LEFT JOIN `dsp_stats_ad_task` AS `ds` ON ds.date BETWEEN '".$where['start_date']."' AND '".$where['end_date']."' AND ds.ad_task_id = dat.borad_id WHERE `dat`.`uid` = '" . $where['uid'] . "' " . (isset($where['shop_id']) ? "AND `dat`.`shop_id`=" . $where['shop_id'] : '') . " AND `dat`.`plat_id` = '134' " . (isset($where['unit_id']) ? "AND `dat`.`unit_id`=" . $where['unit_id'] : '') . " AND `dat`.`is_del` = '0' GROUP BY `dat`.`borad_id` ORDER BY `dat`.`status` DESC LIMIT " . intval(($offset - 1) / $num) . "," . $num;
